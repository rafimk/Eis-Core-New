...
using Eis.Lib.Infrastructure.Configuration;
using Microsoft.Extensions.Logging;

namespace Eis.Lib.Infrastructure;
...
public class MessageQueueManager : IMessageQueueManager
{
            ...
            )
    {
        ...
        eisHostIp = _utility.GetLocalIpAddress();
        ...
        if (inboxEventsList != null && inboxEventsList.Count > 0)
        {
            ...
            foreach (var events in inboxEventsList)
            {
                try
                {
                    ...
                    EisEvent eisEvent = _jsonSerializer.DeserializeObject<EisEvent>(events.EisEvent);
    ...
    private async Task ProcessAllUnProcessedOutboxEvents()
    {
        ...
        if (outboxEventsList != null && outboxEventsList.Count > 0)
        {
            ...
            foreach (var events in outboxEventsList)
            {
                ...
                {
                    _eventID = events.EventId;
                    eisEvent = _jsonSerializer.DeserializeObject<EisEvent>(events.EisEvent);
    ...
    {
        await _utility.ConsumeEventAsync(eisEvent, queueName, _eventRegistry, _log);
    }
