...
using Eis.Lib.Application.Serialization;
using Eis.Lib.Application.Utilities;
...
public class BrokerConnectionFactory : IBrokerConnectionFactory
{
    ...
    private ApplicationOption _appSettings;
    ...
    public BrokerConnectionFactory(IConfigurationManager configurationManager, 
    ...
    private void PublishMessageToTopic(string message)
    {
        ...
        try
        {
            _publisherConnection = _factory.CreateConnection(_brokerOptions.Username, _brokerOptions.Password);
    ...
    public void PublishTopic(EisEvent eisEvent)
    {
        try
        {
            string jsonString = _jsonSerializer.SerializeEvent(eisEvent);
    ...
    public void CreateConsumerListener()
    {
        try
        {
            ...
            _consumerConnection = _factory.CreateConnection(_brokerOptions.Username, _brokerOptions.Password);
    ...
    protected async void OnMessage(IMessage receivedMsg)
    {
        ...
        try
        {
            ...
            eisEvent = _jsonSerializer.DeserializeObject<EisEvent>(queueMessage.Text);
            ...
            if (recordInsertCount == 1)
            {
                ...
                await _utility.ConsumeEventAsync(eisEvent, InboundQueue, _eventRegistry, _log);
